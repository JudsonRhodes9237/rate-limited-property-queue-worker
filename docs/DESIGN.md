...
> Design notes for **rate-limited-property-queue-worker** — a runnable java example that Consume property-management jobs with bounded concurrency, rate limiting, and auditable routing decisions.
...
- **Feature code** — the actual task: property-job-worker.
